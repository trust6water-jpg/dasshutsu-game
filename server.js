...
function handleEvent(event) {
  ...
  if (event.type === 'message' && event.message && event.message.type === 'text') {
    ...
    if (text.includes('持ち物') || text.includes('アイテム')) {
      return sendInventoryFlex(event.replyToken, userId);
    }

    if (text.includes('スタッフそうさ') || text.includes('スタッフ操作')) {
      return sendStaffKanriFlex(event.replyToken);
    }
  ...
  if (event.type === 'postback') {
    ...
    if (action === 'use_item') {
      ...
      return sendCombineSelectFlex(event.replyToken, userId, selectedItem);
    }
...
function sendInventoryFlex(replyToken, userId) {
...
function sendCombineSelectFlex(replyToken, userId, baseItem) {
...
function sendStaffKanriFlex(replyToken) {
  return client.replyMessage(replyToken, {
    type: 'flex',
    altText: '管理室',
    contents: {
